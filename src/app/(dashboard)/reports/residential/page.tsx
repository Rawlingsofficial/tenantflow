...
import {
  ...
  CheckCircle2, Clock, Home, ArrowUpRight, ArrowDownRight
} from 'lucide-react'
...
function StatCard({ label, value, sub, color, icon: Icon, trend }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)] transition-shadow">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${
          ...
          color === 'violet'  ? 'bg-violet-50' : 'bg-gray-50'
        }`}>
          <Icon className={`h-3.5 w-3.5 ${
            color === 'emerald' ? 'text-emerald-500' :
            color === 'amber'   ? 'text-amber-500' :
            color === 'blue'    ? 'text-blue-500' :
            color === 'violet'  ? 'text-violet-500' : 'text-gray-400'
          }`} />
        </div>
      </div>
      <p className={`text-[26px] font-bold tracking-tight leading-none ${
        ...
        color === 'violet'  ? 'text-violet-600' : 'text-gray-800'
      }`}>{value}</p>
      <p className="text-[11px] text-gray-400 mt-1.5">{sub}</p>
      {trend && (
        <div className={`flex items-center gap-0.5 mt-2 text-[10px] font-semibold ${trend.value >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
          {trend.value >= 0
...
function ReportCard({ title, sub, stat, statLabel, href, color, icon: Icon, router }: {
  ...
  const cfg: Record<string, { bg: string; text: string; bar: string; hover: string }> = {
    ...
    violet:  { bg: 'bg-violet-50',  text: 'text-violet-600',  bar: 'bg-violet-500',  hover: 'hover:border-violet-200' },
    amber:   { bg: 'bg-amber-50',   text: 'text-amber-600',   bar: 'bg-amber-500',   hover: 'hover:border-amber-200' },
  }
  const c = cfg[color] ?? cfg.emerald
  return (
    <button onClick={() => router.push(href)}
      className={`bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5 text-left transition-all duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.07)] ${c.hover} group`}>
...
export default function ResidentialReportsPage() {
  ...
  const collectedLastMonth = completedPayments
    ...
    .reduce((s, p) => s + Number(p.amount), 0)
  const revTrend = collectedLastMonth > 0
  ...
  const avgMonthly = Math.round(months12.reduce((s, m) => s + m.collected, 0) / 12)
  ...
  const buildingStats = data.buildings.map(b => {
    ...
    const bLeaseIds = data.leases
      .filter(l => bUnits.some(u => u.id === l.unit_id))
      .map(l => l.id)
    ...
    return {
      ...
      monthly: bMonthly, collected: bCollected,
    ...
    <div className="min-h-screen bg-[#F4F6F9] pb-14">
      ...
      <div className="px-6 grid grid-cols-4 gap-3 mb-5">
        <StatCard label="Portfolio Value" value={`$${portfolioValue.toLocaleString()}`}
          sub="total default rent capacity" color="gray" icon={Building2} />
        <StatCard label="Monthly Run Rate" value={`$${monthlyRunRate.toLocaleString()}`}
          sub={`${activeLeases.length} active leases`} color="blue" icon={TrendingUp} />
        <StatCard label="Collection Rate" value={`${collectionRate}%`}
          ...
          color={collectionRate >= 90 ? 'emerald' : collectionRate >= 70 ? 'amber' : 'amber'}
          icon={CheckCircle2}
        ...
        <StatCard label="Occupancy Rate" value={`${occupancyRate}%`}
          ...
          color={occupancyRate >= 80 ? 'emerald' : 'amber'} icon={Home} />
      </div>
      ...
      <div className="px-6 mb-5">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-6">
          ...
          {/* Bar chart with avg line marker */}
          <div className="relative">
            {/* Avg dashed line */}
            <div className="absolute left-14 right-0 border-t border-dashed border-gray-200 pointer-events-none"
              style={{ top: `${100 - (avgMonthly / maxRevenue) * 100}%`, maxHeight: '100px' }} />

            <div className="flex items-end gap-1.5 h-28">
              {months12.map((m, i) => {
                const h = maxRevenue > 0 ? (m.collected / maxRevenue) * 100 : 0
                const isCurrent = m.ms === thisMonth
                const isHighest = m.collected === Math.max(...months12.map(x => x.collected))
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-0.5 group relative">
                    {m.collected > 0 && (
                      <div className="absolute -top-9 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[10px] px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none shadow-lg">
                        ${m.collected.toLocaleString()}
                      </div>
                    )}
                    <div className="w-full rounded-t-[3px] transition-all duration-300"
                      style={{
                        height: `${Math.max(h, m.collected > 0 ? 5 : 2)}%`,
                  ...
                  <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors cursor-pointer"
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${
                        b.rate >= 90 ? 'bg-emerald-50 text-emerald-700' :
                        b.rate >= 70 ? 'bg-amber-50 text-amber-700' :
        ...
        <div className="px-6 mb-5">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => router.push('/reports/occupancy')}
