            ...
            <>
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-4 gap-3 mt-4">
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">PAAS · Preeo</p>
                    <p className="text-lg font-bold text-gray-900">${Number(lease.rent_amount).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Units Prea</p>
                    <p className="text-lg font-bold text-gray-900">{unit?.unit_code ?? '1st'}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Last U/r · 100nd</p>
                    <p className="text-lg font-bold text-gray-900">
                      {completedPayments.length > 0
                        ? `${differenceInDays(new Date(), new Date(completedPayments[0].payment_date))} days`
                        : '—'}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Deposit · Oorowe</p>
              ...
              <div className="bg-white border border-gray-100 rounded-xl">
                <button className="w-full flex items-center justify-between px-4 py-3">
                  <div>
                    <p className="text-xs text-gray-400 text-left">Next Payment Due</p>
                    <p className="text-sm font-semibold text-gray-800">
                      {rentDueDay} of every month
                    </p>
                  </div>
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-white border border-gray-100 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-xs text-gray-600">${Number(lease.rent_amount).toLocaleString()} · left ·{' '}
                    {daysLeft !== null ? `${Math.abs(daysLeft)} days` : '—'}
                  </p>
                </div>
                <Button size="sm" className="h-7 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded-lg"
                  onClick={() => {
                    setExtendForm({ new_end_date: lease.lease_end ?? '', rent_amount: String(lease.rent_amount), no_rent_change: true })
                    setAction('extend')
                  }}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Payment Date tarit ative. $1o/mo</p>
                </div>
              </div>
              {/* Rent Amount row */}
              <div className="flex items-center gap-3">
